...
/// - For a vertical list, `ScrollSpyAnchor.fraction(0.5)` places the anchor
///   exactly in the vertical center.
/// - `ScrollSpyAnchor.pixels(0)` places it at the very top.
///
/// This anchor is used by:
/// - [ScrollSpyRegion]: To position the line or zone relative to the
///   viewport.
/// - `ScrollSpyPolicy`: To determine which item is "closest" to this point.
@immutable
sealed class ScrollSpyAnchor {
  ...
  const ScrollSpyAnchor({this.offsetPx = 0.0});
  ...
  const factory ScrollSpyAnchor.fraction(double fraction, {double offsetPx}) =
      FractionScrollSpyAnchor;

  /// Creates an anchor defined as a fixed pixel distance from the start of the viewport.
  ///
  /// - `0.0`: Exactly at the start (top/left).
  const factory ScrollSpyAnchor.pixels(
    double pixelsFromStart, {
    double offsetPx,
  }) = PixelsScrollSpyAnchor;
}

/// An anchor positioned at a fractional distance along the viewport's main axis.
///
/// Example: `ScrollSpyAnchor.fraction(0.5)` represents the exact center.
@immutable
final class FractionScrollSpyAnchor extends ScrollSpyAnchor {
  ...
  const FractionScrollSpyAnchor(this.fraction, {super.offsetPx = 0.0})
  ...
  @override
  String toString() =>
      'ScrollSpyAnchor.fraction($fraction, offsetPx: $offsetPx)';
}

/// An anchor positioned at a fixed pixel distance from the viewport's start edge.
///
/// Example: `ScrollSpyAnchor.pixels(100)` places the anchor 100px from the top.
@immutable
final class PixelsScrollSpyAnchor extends ScrollSpyAnchor {
  ...
  const PixelsScrollSpyAnchor(this.pixelsFromStart, {super.offsetPx = 0.0});
  ...
  @override
  String toString() =>
      'ScrollSpyAnchor.pixels($pixelsFromStart, offsetPx: $offsetPx)';
}

/// Data passed to [ScrollSpyRegion.evaluate] for a specific item.
///
...
/// The result of evaluating whether an item intersects a [ScrollSpyRegion].
@immutable
class ScrollSpyRegionResult {
  ...
  static const ScrollSpyRegionResult notFocused = ScrollSpyRegionResult(
    isFocused: false,
    focusProgress: 0.0,
    overlapFraction: 0.0,
  );
...
typedef ScrollSpyRegionEvaluator = ScrollSpyRegionResult Function(
    ScrollSpyRegionInput input);
...
@immutable
sealed class ScrollSpyRegion {
  const ScrollSpyRegion();
  ...
  /// compute pass. The returned [ScrollSpyRegionResult]:
  ...
  /// Built-in regions never allocate here; [ScrollSpyRegion.custom] builds a
  /// [ScrollSpyRegionInput] via [itemRect] and delegates to its evaluator
  /// (allocation is the documented cost of custom regions).
  @internal
  ...
  const factory ScrollSpyRegion.line({
    required ScrollSpyAnchor anchor,
    double thicknessPx,
  }) = ScrollSpyLineRegion;
  ...
  const factory ScrollSpyRegion.zone({
    required ScrollSpyAnchor anchor,
  ...
  const factory ScrollSpyRegion.custom({
    required ScrollSpyAnchor anchor,
    required ScrollSpyRegionEvaluator evaluator,
  }) = ScrollSpyCustomRegion;
}
...
final class ScrollSpyLineRegion extends ScrollSpyRegion {
  ...
  const ScrollSpyLineRegion({required this.anchor, this.thicknessPx = 0.0})
  ...
  void evaluateMainAxisInto(
    ...
    out.overlapFraction = thicknessPx == 0.0
        ? 1.0
        : (overlapPx / thicknessPx).clamp(0.0, 1.0);
  }
...
final class ScrollSpyZoneRegion extends ScrollSpyRegion {
  ...
  const ScrollSpyZoneRegion({required this.anchor, required this.extentPx})
      : assert(extentPx > 0.0);
  ...
  ScrollSpyRegionResult evaluate(ScrollSpyRegionInput input) {
    ...
    if (!isFocused) {
      return ScrollSpyRegionResult.notFocused;
    }
...
final class ScrollSpyCustomRegion extends ScrollSpyRegion {
  ...
  @override
  String toString() =>
      'ScrollSpyRegion.custom(anchor: $anchor, evaluator: $evaluator)';
}
