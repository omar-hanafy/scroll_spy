...
/// [ScrollSpyController].
///
/// Developers typically consume this object in the builder of a [ScrollSpyItem] to:
...
@immutable
class ScrollSpyItemFocus<T> {
  /// A stable identifier for the item (post id, key, database ID, etc).
  ///
  /// This ID must be unique among all registered items in the [ScrollSpyScope].
  final T id;
  ...
  /// Whether this item intersects the active [ScrollSpyRegion].
  ///
  ...
  ///
  /// The primary item is determined by the [ScrollSpyPolicy] (e.g., closest to center)
  /// and filtered by [ScrollSpyStability] rules (hysteresis, min duration).
  ///
  ...
  /// The signed distance (in pixels) from the center of this item to the configured [ScrollSpyAnchor].
  ///
  ...
  ///
  /// This metric is the primary driver for [ScrollSpyPolicy.closestToAnchor].
  ...
  ///
  /// - For a `ScrollSpyRegion.line` with `thicknessPx == 0`, this is `1.0`
  ...
  /// - For `ScrollSpyRegion.zone`, this is the fraction of the zone band
  ///   overlapped by the item (`overlapPx / extentPx`, clamped).
  ///
  /// This is the primary driver for [ScrollSpyPolicy.largestFocusOverlap].
  ...
  factory ScrollSpyItemFocus.unknown({required T id}) {
  ...
  bool operator ==(Object other) {
    return other is ScrollSpyItemFocus<T> &&
        other.id == id &&
...
class ScrollSpySnapshot<T> {
  ...
  /// pass).
  final Map<T, ScrollSpyItemFocus<T>> items;
  ...
  ScrollSpySnapshot({
    ...
    required Map<T, ScrollSpyItemFocus<T>> items,
  ...
  factory ScrollSpySnapshot.empty({DateTime? computedAt}) {
  ...
  ScrollSpyItemFocus<T>? itemOf(T id) => items[id];
  ...
  ScrollSpySnapshot<T> copyWith({
    DateTime? computedAt,
    T? primaryId,
    Set<T>? focusedIds,
    Set<T>? visibleIds,
    Map<T, ScrollSpyItemFocus<T>>? items,
  }) {
    return ScrollSpySnapshot<T>(
  ...
  @override
  bool operator ==(Object other) {
    return other is ScrollSpySnapshot<T> &&
